...
class AppointmentModel {
     ...
     */
    public function validateData($data, $appointmentId = null) {
        ...
        $this->validateUnique('appointments', 'CURP', $data['CURP'], $appointmentId, "La CURP que intentas registrar ya existe.");
        $this->validateUnique('appointments', 'phone', $data['phone'], $appointmentId, "El número de teléfono que intentas registrar ya existe.");
        $this->validateUnique('appointments', 'insurance_number', $data['insurance_number'], $appointmentId, "El número de afiliación que intentas registrar ya existe.");
    }
     ...
     */
    public function updateAppointment($appointmentId, $data, $photoData) {
        // Verifica que el doctor exista
        $stmt = $this->pdo->prepare("SELECT id FROM appointments WHERE id = :appointment_id");
        $stmt->execute([':doctor_id' => $appointmentId]);
        if (!$stmt->fetch(PDO::FETCH_ASSOC)) {
            throw new Exception("El doctor con ID $appointmentId no existe.");
        }
