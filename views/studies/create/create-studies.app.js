document.addEventListener("DOMContentLoaded", () => {
  ...
  // ================================
  const windowsContainer = document.getElementById(
    "operational-windows-container"
  );
  ...
  // ================================
  // Mostrar/ocultar select de máquina
  // ================================
  const requiresMachineCheckbox = document.getElementById("requires_machine");
  const machineSelectGroup = document.getElementById("machine_select_group");

  if (requiresMachineCheckbox && machineSelectGroup) {
    // Mostrar/ocultar según el estado inicial
    machineSelectGroup.style.display = requiresMachineCheckbox.checked
      ? "block"
      : "none";

    // Cambiar visibilidad al marcar/desmarcar
    requiresMachineCheckbox.addEventListener("change", () => {
      machineSelectGroup.style.display = requiresMachineCheckbox.checked
        ? "block"
        : "none";
    });
  ...
  forms.forEach((form) => {
    form.addEventListener("submit", (e) => {
      ...
      if (name !== undefined && name.length < 3)
        errors.push(
          "El nombre de la máquina debe tener al menos 3 caracteres."
        );
      ...
      if (area !== undefined && !area)
        errors.push("Debe seleccionar un área médica.");
      if (status !== undefined && !status)
        errors.push("Debe seleccionar un estado operativo.");
        ...
        if (!start || !end)
          errors.push(
            `La ventana operativa #${
              index + 1
            } debe tener hora de inicio y fin.`
          );
        if (start && end && start >= end)
          errors.push(
            `En la ventana operativa #${
              index + 1
            }, la hora de inicio debe ser menor que la hora de fin.`
          );
      });

      // Validación: si requiere máquina, debe seleccionar una
      if (requiresMachineCheckbox?.checked) {
        const selectedMachine = form.querySelector("#machine_id")?.value;
        if (!selectedMachine)
          errors.push(
            "Debe seleccionar una máquina si marcó 'Requiere máquina'."
          );
      }
